...
function removeFirstOccurrences(str, value) {
  // return str.slice(str.indexOf(value), value.length);
  const firstIndex = str.indexOf(value);
  if (firstIndex === -1) {
    // console.log(str);
    return str;
  }
  // console.log(str.slice(0, firstIndex) + str.slice(firstIndex + value.length));
  return str.slice(0, firstIndex) + str.slice(firstIndex + value.length);
}
...
function sumOfCodes(str) {
  ...
  let answer = 0;
  // str.split('').forEach((el) => (answer += el.charCodeAt(0)));.
  // for (let i = 0; i < str.split('').length; i += 1) {
  //   answer += str.split('')[i].charCodeAt(0);
  // }
  // return answer;

  const arr = str.split('');
  for (let i = 0; i < arr.length; i += 1) {
    answer += arr[i].charCodeAt(0);
  }
  return answer;
}
...
function endsWith(str, substr) {
  ...
  //   .reverse()
  //   .join('')
  //   .startsWith(substr.split('').reverse().join(''));
...
function orderAlphabetically(str) {
  return str === '' ? str : str.split('').sort().join('');
}
...
function countVowels(str) {
  ...
  return str.match(/[aeiouyAEIOUY]/gi)
    ? str.match(/[aeiouyAEIOUY]/gi).length
    : 0;
}
...
function findLongestWord(sentence) {
  // return Math.max(...sentence.split(' ').map((el) => parseInt(el.length, 10)));
...
function reverseWords(str) {
  // let words = str.split(" ");
  // console.log(words.map((word) => word.split("").reverse().join("")).join(" "));
  return str
    .split(' ')
...
function encodeToRot13(str) {
  ...
  const answer = [];
  const togetherArr = mainArr.join('');
  const word = str.split('');

  function contrChar(n) {
    if (n > 13) {
      return n - 13;
    }
    return n + 13;
  ...
  word.forEach((el) => {
    ...
    if (togetherArr.includes(char)) {
      index = togetherArr.indexOf(char);
      if (el === char) {
        answer.push(mainArr[contrChar(index)]);
      } else {
        answer.push(mainArr[contrChar(index)].toUpperCase());
      }
    } else {
      answer.push(char);
    }
